...
import { deletePodcastEpisode, savePodcastEpisode } from './actions';
...
export function PodcastsClient({ episodes, team }: PodcastsClientProps) {
  const [editing, setEditing] = useState<PodcastEpisodeRow | 'new' | null>(null);
...
type EditFormValues = PodcastEpisodePayload;

function PodcastEpisodeEditor({
  open,
  episode,
  team,
  onClose,
  ...
  open: boolean;
  episode: PodcastEpisodeRow | null;
  team: TeamMemberOption[];
  onClose: () => void;
}) {
  ...
  const onDelete = () => {
    ...
    startTransition(async () => {
      const result = await deletePodcastEpisode(episode.id);
      if (!result.ok) {
        setServerError(result.error);
        return;
      }
  ...
  return (
    <Modal
      ...
      crumbs={episode ? `Édition · ${episode.title}` : 'Nouvel épisode podcast'}
    ...
    >
      <form id="podcast-episode-form" onSubmit={handleSubmit(onSubmit)} noValidate>
