...
| 13 | Bento hebdomadaire | Rétention | L | 15, 16 | 🟡 **spécification écrite et validée le 16/09**, dix décisions · ~~la PK `(bento_id, category_id)` interdit deux cases du même type~~ **faux depuis le chantier 15** : `bento_categories` est la table des cases, deux cases « film » sont deux lignes · le vrai blocage est que les six cases sont compilées dans l'app, 24 fichiers hors tests · **ne peut pas sortir avant la migration B**, donc avant une sortie store · [spec](./UX-13-BENTO-HEBDOMADAIRE.md) |
